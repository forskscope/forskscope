...
use forskscope_core::command::{
    Availability, CommandContext, CommandDefinition, CommandRegistry, UnavailableReason, cmd,
};
...
pub fn build_toolbar(registry: &CommandRegistry, ctx: &CommandContext) -> Vec<ToolbarSection> {
    ...
    vec![
        ...
        ToolbarSection {
            label: "Merge",
            items: [
                &cmd::COPY_HUNK_LEFT_RIGHT,
                &cmd::COPY_HUNK_RIGHT_LEFT,
                &cmd::USE_LEFT,
                &cmd::USE_RIGHT,
            ]
            .iter()
...
mod tests {
    ...
    fn enabled_count_zero_in_empty_context() {
        ...
        assert!(
            enabled_count(&sections) >= 2,
            "at least the Always commands must be enabled"
        );
    }
    ...
    fn all_toolbar_items_have_non_empty_labels() {
        ...
        for section in &sections {
            for item in &section.items {
                assert!(
                    !item.label.is_empty(),
                    "item {} must have a non-empty label",
                    item.command_id
                );
            }
        }
    ...
    fn enabled_item_has_disabled_reason_none() {
        ...
        assert!(
            palette.disabled_reason.is_none(),
