...
pub fn build_palette(
    ...
    let mut rows: Vec<PaletteRow> = filtered
        .into_iter()
        .map(|def| {
...
mod tests {
    ...
    fn query_save_returns_save_commands() {
        ...
        for row in &rows {
            ...
            assert!(
                ...
                "row {:?} matched 'save' but label+description don't contain it",
                row.label
            );
        }
    }
    ...
    fn enabled_rows_come_before_disabled() {
        ...
        for row in &rows {
            ...
            if saw_disabled {
                assert!(
                    !row.enabled,
                    "enabled row {:?} appeared after disabled rows",
                    row.label
                );
            }
        }
    ...
    fn next_difference_enabled_in_diff_context() {
        ...
        let next = rows
            .iter()
            .find(|r| r.command_id == "navigate.next_difference")
    ...
    fn all_rows_have_non_empty_labels() {
        ...
        for row in &rows {
            assert!(
                !row.label.is_empty(),
                "command {} must have non-empty label",
                row.command_id
            );
        }
    }
    ...
    fn save_row_has_ctrl_s_shortcut_hint() {
        ...
        assert!(
            hint.to_lowercase().contains('s'),
            "file.save shortcut hint '{}' must reference 's'",
            hint
        );
    }
    ...
    fn disabled_row_has_disabled_reason_some() {
        ...
        assert!(!save.enabled, "file.save must be disabled in empty context");
        assert!(
            save.disabled_reason.is_some(),
            "disabled_reason must be Some when the command is disabled"
        );
        assert!(
            !save.disabled_reason.unwrap().is_empty(),
    ...
    fn enabled_row_has_disabled_reason_none() {
        ...
        assert!(
            palette.is_some(),
            "command_palette must appear for 'palette' query"
        );
        let row = palette.unwrap();
        assert!(
            row.enabled,
            "view.command_palette must be enabled in any context"
        );
        assert!(
    ...
    fn all_rows_have_non_empty_descriptions() {
        ...
        for row in &rows {
            assert!(
                ...
                "description must be non-empty for command '{}'",
                row.command_id
            );
        }
    }
