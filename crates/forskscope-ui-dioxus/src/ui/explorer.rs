//! Explorer workspace coordination (RFC-005, RFC-021, RFC-037).
...
pub enum ExplorerFilter { #[default] All, Different, Equal }

#[derive(Clone, Copy, PartialEq, Eq, Default)]
#[allow(clippy::enum_variant_names)]
pub enum SortMode { #[default] ByName, ByStatus, BySize }
...
pub fn Explorer() -> Element {
    ...
    let mut filter:  Signal<ExplorerFilter> = use_signal(ExplorerFilter::default);
    ...
    rsx! {
        div { class: "explorer",
            ...
            div { class: "explorer-toolbar",
                // Filter
                button {
                    class: if f == ExplorerFilter::All { "filter-btn active" } else { "filter-btn" },
                    onclick: move |_| filter.set(ExplorerFilter::All), "All"
                }
                button {
                    class: if f == ExplorerFilter::Different { "filter-btn active" } else { "filter-btn" },
                    onclick: move |_| filter.set(ExplorerFilter::Different), "Different"
                }
                button {
                    class: if f == ExplorerFilter::Equal { "filter-btn active" } else { "filter-btn" },
                    onclick: move |_| filter.set(ExplorerFilter::Equal), "Equal"
                }
                span { class: "spacer" }
                // Sort
                span { class: "sort-label", "Sort:" }
                select {
                    onchange: move |e| sort.set(match e.value().as_str() {
                        "status" => SortMode::ByStatus, "size" => SortMode::BySize, _ => SortMode::ByName,
                    }),
                ...
                button {
                    ...
                    title: "Scan both directories recursively and list all differences",
                    onclick: move |_| {
                        let m = *mode.read();
                        mode.set(if m == ExplorerMode::Deep { ExplorerMode::Browse } else { ExplorerMode::Deep });
                    },
                    "⟳ Deep compare"
                }
            }

            if *mode.read() == ExplorerMode::Deep {
                // ── Recursive deep compare view ───────────────────────────────
                DeepCompareView {
                    left_root:  left_dir.read().clone(),
                    right_root: right_dir.read().clone(),
                    lang,
                }
            } else {
                // ── Browse mode: two directory panes ─────────────────────────
                DirPane {
...
struct DirSummary { common: usize, changed: usize, left_only: usize, right_only: usize }

fn compute_summary(l: &HashSet<String>, r: &HashSet<String>,
                   d: &HashMap<String, DigestState>) -> DirSummary {
    let mut s = DirSummary { common: 0, changed: 0, left_only: 0, right_only: 0 };
    let all: HashSet<&String> = l.iter().chain(r.iter()).collect();
    for name in all {
        match (l.contains(name), r.contains(name)) {
            (true, true) => { s.common += 1; if matches!(d.get(name), Some(DigestState::Changed)) { s.changed += 1; } }
