...
fn list_dir_io_is_recoverable_severity() {
    assert_eq!(
        io(IoOperation::ListDir).severity(),
...
fn internal_invariant_is_blocking_severity() {
    let e = CoreError::InternalInvariant {
        message: "bug".into(),
    };
...
fn read_io_hints_choose_another_file() {
    assert_eq!(
        io(IoOperation::Read).recovery_hint(),
        RecoveryHint::ChooseAnotherFile
    );
}
...
fn rename_io_hints_save_as() {
    assert_eq!(
        io(IoOperation::Rename).recovery_hint(),
        RecoveryHint::SaveAs
    );
}
...
fn copy_io_hints_check_permissions() {
    assert_eq!(
        io(IoOperation::Copy).recovery_hint(),
