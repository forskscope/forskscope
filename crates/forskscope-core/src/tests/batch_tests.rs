...
fn tmp(tag: &str) -> PathBuf {
    let d = std::env::temp_dir().join(format!("fsk-batch-{tag}-{}", std::process::id()));
...
fn stop_on_first_marks_remaining_as_skipped() {
    ...
    let items = vec![
        BatchItem {
            src: src.join("missing.txt"),
            dst: dst.join("missing.txt"),
        },
        BatchItem {
            src: src.join("good.txt"),
            dst: dst.join("good.txt"),
        },
    ];
    let m = batch_copy(
        &items,
    ...
    assert!(
        !dst.join("good.txt").exists(),
        "skipped item must not be copied"
    );
    let _ = fs::remove_dir_all(&base);
}
...
fn continue_on_failure_collects_all_outcomes() {
    ...
    let items = vec![
        BatchItem {
            src: src.join("ok1.txt"),
            dst: dst.join("ok1.txt"),
        },
        BatchItem {
            src: src.join("missing.txt"),
            dst: dst.join("missing.txt"),
        ...
        BatchItem {
            src: src.join("ok2.txt"),
            dst: dst.join("ok2.txt"),
        },
    ];
    let m = batch_copy(
        &items,
    ...
    assert!(
        !m.entries
            .iter()
            .any(|e| matches!(e.outcome, EntryOutcome::Skipped { .. }))
    );
    assert!(
        dst.join("ok2.txt").exists(),
        "ok2 should be copied despite earlier failure"
    );
    let _ = fs::remove_dir_all(&base);
}
...
fn manifest_json_contains_operation_id_and_entry_outcomes() {
    ...
    assert!(
        ...
        "must have operation_id field"
    );
    assert!(
        json.contains("\"app_version\""),
        "must have app_version field"
    );
    assert!(json.contains("\"entries\""), "must have entries array");
    assert!(
        json.contains("\"copied\""),
        "successful copy must show outcome=copied"
    );
...
fn operation_id_is_unique_across_two_calls() {
    ...
    assert!(
        id1.0.starts_with("op-"),
        "id must start with op-: {}",
        id1.0
    );
    assert!(
        id2.0.starts_with("op-"),
        "id must start with op-: {}",
        id2.0
    );
}
