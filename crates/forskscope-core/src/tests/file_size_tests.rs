...
fn default_limits_are_ordered_ascending() {
    ...
    assert!(
        l.max_eager_text_bytes < l.medium_text_threshold_bytes,
        "small < medium"
    );
    assert!(
        l.medium_text_threshold_bytes < l.large_text_threshold_bytes,
        "medium < large"
    );
}
...
fn zero_bytes_is_small() {
    assert_eq!(
        FileSizeClass::classify(0, &PerformanceLimits::default()),
        FileSizeClass::Small
    );
}
...
fn at_max_eager_boundary_is_small() {
    let l = PerformanceLimits::default();
    assert_eq!(
        FileSizeClass::classify(l.max_eager_text_bytes, &l),
        FileSizeClass::Small
    );
}
...
fn one_byte_above_eager_boundary_is_medium() {
    let l = PerformanceLimits::default();
    assert_eq!(
        FileSizeClass::classify(l.max_eager_text_bytes + 1, &l),
...
fn at_medium_boundary_is_medium() {
    let l = PerformanceLimits::default();
    assert_eq!(
        FileSizeClass::classify(l.medium_text_threshold_bytes, &l),
        FileSizeClass::Medium
    );
}
...
fn one_byte_above_medium_is_large() {
    let l = PerformanceLimits::default();
    assert_eq!(
        FileSizeClass::classify(l.medium_text_threshold_bytes + 1, &l),
        FileSizeClass::Large
    );
}
...
fn at_large_boundary_is_large() {
    let l = PerformanceLimits::default();
    assert_eq!(
        FileSizeClass::classify(l.large_text_threshold_bytes, &l),
        FileSizeClass::Large
    );
}
...
fn above_large_boundary_is_very_large() {
    let l = PerformanceLimits::default();
    assert_eq!(
        FileSizeClass::classify(l.large_text_threshold_bytes + 1, &l),
...
fn huge_file_is_very_large() {
    let l = PerformanceLimits::default();
    assert_eq!(
        FileSizeClass::classify(1024 * 1024 * 1024, &l),
        FileSizeClass::VeryLarge
    );
}
...
fn custom_limits_classify_correctly() {
    ...
    assert_eq!(
        FileSizeClass::classify(1_000, &limits),
        FileSizeClass::Small
    );
    assert_eq!(
        FileSizeClass::classify(1_001, &limits),
        FileSizeClass::Medium
    );
    assert_eq!(
        FileSizeClass::classify(10_001, &limits),
        FileSizeClass::Large
    );
    assert_eq!(
        FileSizeClass::classify(100_001, &limits),
        FileSizeClass::VeryLarge
    );
}
