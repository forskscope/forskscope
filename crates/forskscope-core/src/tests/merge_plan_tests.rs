...
use crate::dir::{
    BatchFailurePolicy, CopyDirection, DirectoryMergeAction, EntrySelection, FileOutcome, RecEntry,
    RecStatus, execute_plan, plan_operations,
};
...
fn tmp(tag: &str) -> PathBuf {
    let d = std::env::temp_dir().join(format!("fsk-mergeplan-{tag}-{}", std::process::id()));
...
fn changed_entry_is_planned_for_copy_in_both_directions() {
    ...
    assert_eq!(
        l2r.operations[0].action,
        DirectoryMergeAction::CopyLeftToRight
    );

    let r2l = plan_operations(
        &entries,
        "/l".as_ref(),
        "/r".as_ref(),
        CopyDirection::RightToLeft,
    ...
    assert_eq!(
        r2l.operations[0].action,
        DirectoryMergeAction::CopyRightToLeft
    );
}
...
fn equal_entries_are_excluded_from_plan() {
    ...
    assert_eq!(
        plan.risk_summary.total_files, 1,
        "equal file must not count toward total"
    );
...
fn left_only_entry_is_skipped_when_direction_is_right_to_left() {
    ...
    assert_eq!(
        plan.risk_summary.total_files, 0,
        "skip doesn't count as copy"
    );
}
...
fn changed_only_filter_excludes_one_sided_entries() {
    ...
    let plan = plan_operations(
        ...
        "/r".as_ref(),
        CopyDirection::LeftToRight,
        EntrySelection::ChangedOnly,
    );
    assert_eq!(
        plan.risk_summary.total_files, 1,
        "only changed files should be planned"
    );
}
...
fn source_only_filter_includes_only_source_side_entries() {
    ...
    let plan = plan_operations(
        ...
        "/r".as_ref(),
        CopyDirection::LeftToRight,
        EntrySelection::SourceOnlyEntries,
    );
    assert_eq!(plan.risk_summary.total_files, 1);
    assert_eq!(
        plan.operations
            .iter()
            .filter(|o| o.action != DirectoryMergeAction::Skip)
            .count(),
        1
    );
}
...
fn risk_summary_counts_are_accurate() {
    ...
    assert_eq!(
        plan.risk_summary.overwrites, 1,
        "existing.txt is an overwrite"
    );
...
fn preflight_detects_existing_target() {
    ...
    let op = &plan.operations[0];
    assert!(
        op.preflight.target_exists,
        "existing target must be detected"
    );
