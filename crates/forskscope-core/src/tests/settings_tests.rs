...
use crate::settings::{
    Density, FontFamilySetting, LocaleId, SETTINGS_SCHEMA_VERSION, ThemeId, UserSettings,
};
...
fn theme_css_var_names_returns_twelve_entries_per_theme() {
    ...
    for theme in ThemeId::all() {
        assert_eq!(
            ThemeTokens::css_var_names(*theme).len(),
            12,
...
fn css_var_names_all_start_with_fsk_prefix() {
    ...
    for (var, _) in ThemeTokens::css_var_names(ThemeId::Dark) {
        assert!(
            var.starts_with("--fsk-"),
            "CSS var {var} must have --fsk- prefix"
        );
    }
}
...
fn newer_schema_version_returns_error() {
    ...
    assert!(
        UserSettings::from_json(&bumped).is_err(),
...
fn corrupt_json_falls_back_to_defaults_not_error() {
    ...
    assert!(
        result.is_ok(),
        "corrupt payload must fall back to defaults, not error"
    );
...
fn font_size_is_clamped_on_load() {
    ...
    assert!(
        loaded.appearance.font_size <= 50,
