...
impl AvailabilityRule {
    ...
    pub fn evaluate(self, ctx: &CommandContext) -> Availability {
        let (ok, reason) = match self {
            Self::Always => (true, ""),
            Self::ActiveDiffTab => (ctx.has_active_diff_tab, "No comparison is open"),
            Self::DirtyAndSaveable => (
                ctx.active_tab_is_dirty && ctx.active_tab_is_saveable,
                "Nothing to save",
            ),
            Self::ActiveCompareTab => (ctx.has_active_compare_tab, "No file is open"),
            Self::ActiveHunkEditable => (
                ctx.active_hunk_exists && ctx.right_side_is_editable,
                "No editable hunk is focused",
            ),
            Self::HasHunks => (ctx.active_tab_has_hunks, "No differences found"),
            Self::ActiveConflict => (ctx.has_active_conflict, "No conflict is selected"),
            Self::AnyConflictUnresolved => {
                (ctx.any_conflict_unresolved, "All conflicts are resolved")
            }
            Self::CanUndo => (ctx.can_undo, "Nothing to undo"),
            Self::CanRedo => (ctx.can_redo, "Nothing to redo"),
            Self::SelectedPathExists => (ctx.selected_path_exists, "No file is selected"),
        };
        if ok {
