...
fn load(path: &str) -> String {
    ...
    let full = Path::new(&manifest).join("../../tests/fixtures").join(path);
    fs::read_to_string(&full).unwrap_or_else(|e| panic!("cannot read fixture {path}: {e}"))
}

fn opts_default() -> DiffOptions {
    DiffOptions::default()
}
...
fn identical_fixture_produces_no_changes() {
    ...
    assert!(
        doc.is_identical(),
        "identical fixture must produce no changed hunks"
    );
...
fn one_changed_line_produces_one_replace_hunk() {
    ...
    let left_text: Vec<&str> = hunk
        .rows
        .iter()
        .filter_map(|r| r.left.as_ref().map(|l| l.content.trim()))
        .collect();
    let right_text: Vec<&str> = hunk
        .rows
        .iter()
...
fn insertions_fixture_produces_insert_hunks() {
    ...
    assert!(
        has_insert,
        "insertions fixture must contain at least one Insert hunk"
    );
...
fn deletions_fixture_produces_delete_hunks() {
    ...
    assert!(
        has_delete,
        "deletions fixture must contain at least one Delete hunk"
    );
...
fn empty_vs_nonempty_is_pure_insert() {
    ...
    assert!(
        all_inserts,
        "empty vs nonempty must produce only Insert hunks"
    );
}
...
fn unicode_content_equal_with_ignore_case() {
    ...
    assert!(
        doc.is_identical(),
        "world vs WORLD must be equal with ignore_case"
    );
}
...
fn large_equal_files_are_identical() {
    ...
    assert!(
        doc.is_identical(),
        "200-line identical files must produce no changes"
    );
}
...
fn large_file_with_one_change_produces_one_hunk() {
    ...
    assert_eq!(
        doc.stats.lines_inserted + doc.stats.lines_deleted,
        2,
...
fn binary_fixture_classifies_as_binary() {
    ...
    let manifest =
        std::env::var("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR must be set by cargo test");
    let path = Path::new(&manifest).join("../../tests/fixtures/text/binary_nul.bin");
...
fn text_fixtures_classify_as_text() {
    ...
    for name in &["left_identical.txt", "left_unicode.txt", "utf8_bom.txt"] {
        ...
        assert_eq!(kind, FileKind::Text, "fixture {name} must classify as Text");
    }
}
...
fn very_long_single_line_produces_one_replace_hunk() {
    ...
    assert!(
        !diff.hunks.is_empty(),
        "long single-line diff must not be empty"
    );
