...
| F17 | Windows release build fails: `app-json-settings` (2.3.0 and 2.4.0, the latest available) has an out-of-scope `use std::os::windows::ffi::OsStrExt` in `replace_file`'s local scope that does not cover the sibling `wide_null_terminated` function, which also calls `.encode_wide()`. Upstream fix required in `github.com/nabbisen/app-json-settings-rs`; not fixable within this repository. Discovered by R0's first real release-workflow run — this is why R0 required an observed run rather than a configuration review. | 2026-08-01 R0 release run | before next Windows-inclusive release |
