...
| F38 | `persist_noclobber` sets `0o644` unconditionally while `atomic_replace` uses `fs::write` (`0666 & ~umask`), so one product has two save paths with different permission behaviour. A user with `umask 077` gets `0600` from a normal save and `0644` from the mergetool no-clobber path — more permissive than their environment asked for, in a tool used on credentials and production logs. **Direction settled (review 051): umask-derived.** `persist_noclobber` runs only for `MustBeAbsent`, so there is never an existing mode to preserve — that option is inapplicable by construction, and belongs to F9's overwrite case instead. Property: a file created by the no-clobber path has the permissions it would have had via the normal save path; assert equivalence with an `fs::write`-created file rather than the literal `0o644` the current test pins. Adjacent to F9 but distinct: F9 is preserved files, this is newly created ones | review 048 | before M3 closes |
