...
| F61 | **Resolved.** Root cause (established via F62's now-visible error, then confirmed by toggling the one variable in an isolated regression test): **nothing in the app ever creates `~/.config/forskscope/` (or the platform equivalent) before the first write into it.** `atomic_write_envelope`'s `atomic_replace` writes a temp file in the same directory as its target and renames it — if that directory doesn't exist yet (a genuinely fresh install, or any config path with no prior settings/session file), the temp-file write fails with `NotFound`, which F62 was silently swallowing. Not a UI-wiring bug at all — the reactive `use_effect` was firing correctly the whole time; the write beneath it was failing. Fixed at the repository/path layer, per the handoff's guidance, not with a compensating call in `app.rs`: `atomic_write_envelope` (`persist/schema/repository.rs`) now calls `fs::create_dir_all` on the target's parent directory before writing. `ensure_pre_v2_backup`/`ensure_reset_backup` don't need the same fix — they only ever run against a file that was just successfully read, so its parent directory is already known to exist. Regression test: `app::tests::a_cli_opened_tab_is_persisted_to_session_json` renders the real `App()` component (not `with_test_store`, which cannot reach `app.rs`'s reactive effect at all — a real finding about F36's harness, noted where the test lives) through a real `VirtualDom` with a genuine CLI `Compare` startup request, and asserts `session.json` is written. `#[ignore]`d: reliable only at `--test-threads<=2` (confirmed empirically: passes 5/5 at N≤2, fails 5/5 with a real 10s timeout at every N≥3 tried including the ~32-thread default, regardless of which other tests run alongside it, even alone) — a genuine, unresolved `dioxus-core`/`tokio` test-infrastructure limitation distinct from and unrelated to the fix's own correctness, documented rather than hidden per the handoff's explicit permission. `README.md:96`'s "Session persistence — open tabs are restored on next launch" claim is now true on both paths; not narrowed since the fix makes it true rather than requiring a scope reduction | M5-B, found independently on Linux/Windows/macOS, 2026-08-15; root cause established and fixed, 2026-08-16 | **done — new candidate needed to re-run M5's P12 rows and clear as a Gate D blocker** |
