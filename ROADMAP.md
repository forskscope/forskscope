...
| F62 | **`persist_session` and `persist_settings` discard their write `Result` with `let _ =`, so every persistence failure is silent by construction.** Distinct from F61: F61 is a write that never happens, F62 is why that and any future write failure produce no error, no toast, and no log. Fixing F61 without this leaves the next persistence failure just as invisible. Found during M5-B's macOS investigation | review 064 | with F61's fix |
| F61 | **A tab opened via CLI startup args is not reliably persisted to `session.json` — confirmed independently on all three M5-B platforms, then reproduced a fourth time directly on a real desktop (review 064).** RFC-078 P12 (session restore) fails the same way on Linux, Windows, and macOS: launch with `forskscope <left> <right>`, terminate, relaunch with no args, and the tab does not restore. `app.rs`'s reactive `use_effect` on `store.tabs` is meant to cover exactly this (it's what makes settings/session persistence work "reactively, not only on exit" elsewhere), but the macOS M5-B harness traced the actual gap: that `use_effect` never fires a real write for a CLI-opened tab, while an explicit call site like `close_tab` does persist correctly — so a session opened then cleanly closed saves fine, but one left open across a terminate does not. **Un-waivable (review 064 §4): RFC-078's Waiver policy names "silent settings/session loss" as one of five things no waiver may turn into a release pass, and this is that, exactly** — `forskscope <left> <right>`, the ordinary `git difftool` workflow, opens a session that is silently gone on the next launch. A second, independent Gate D blocker alongside F44 — unlike F44, this is a defect the project itself can fix, not a schedule dependency on an upstream release. Not fixed in M5-B (its constraints forbade product changes; register only) — **F61's fix is its own slice against a later candidate, not evidence work; the architect is writing that handoff separately.** One consequence: the top-level `README.md`'s "Session persistence — open tabs are restored on next launch" claim is false for a CLI-opened tab (F16's earlier feature-claim audit couldn't have caught this — it audited against the UI crate, and this claim holds on one path and fails on another). Windows's M5-B P12 evidence initially read Pass because it worked around F61 by seeding `session.json` directly instead of exercising the CLI-launch path; corrected to Fail (review 064 §5.4) so the evidence doesn't misrepresent a cross-platform defect as platform-specific. Evidence in each platform's `docs/src/maintainers/release-evidence/0.167.0/{linux-x11,windows-11,windows-10,macos-aarch64}.md`, and in the shared `README.md`'s verdict alongside F44 | M5-B, found independently on Linux/Windows/macOS, 2026-08-15; reproduced review 064, 2026-08-16 | **un-waivable Gate D blocker; fix scheduled as its own slice** |
