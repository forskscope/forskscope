...
| F33 | README and user docs have no installation path — both say "build from source" while `0.165.0` publishes four artifacts plus an AUR PKGBUILD and a Microsoft Store listing that appears nowhere in the docs. No screenshots exist either. Add a compact Linux-first install section, a `users/installation.md`, and screenshots. **Deferred until F32 lands** so the images match what a user downloads | owner request, 2026-08-03 | after F32 |
...
| F44 | **The published `linux-x86_64` binary does not start on any distro shipping libxdo 4** (Arch/CachyOS confirmed): `error while loading shared libraries: libxdo.so.3`. Of its 146 dynamic dependencies exactly one is unresolved — WebKitGTK 4.1 and GTK 3 sonames are stable across distros, `libxdo` is not. Built on `ubuntu-latest`, which ships soname 3; rolling distros ship 4, so **installing `xdotool` does not fix it** and the artifact is effectively Debian/Ubuntu-only while being advertised as `linux-x86_64`. Not a dev/test dependency: `libxdo` is a **default** feature of `muda` (menu-accelerator key synthesis), and `dioxus-desktop` depends on `muda` without `default-features = false`, so it cannot be disabled from this workspace — Cargo feature unification is additive. Compounding it, the runtime dependency was only ever tracked as a *build* dependency: `local-dev.md` lists `libxdo-dev`, but `quick-start.md`, `troubleshooting.md` and PKGBUILD's `depends=()` all name only WebKitGTK and GTK3. Options: ask upstream Dioxus to set `default-features = false` on `muda` (the app has no menu bar, so the feature is dead weight — same route as the `app-json-settings` F17 fix); or bundle the library with an rpath; or state a supported runtime baseline and stop labelling the artifact `linux-x86_64`. Found by the owner running the artifact — nothing in CI launches it, which is F34 | owner, post-0.166.0 | **0.166.1** |
