...
| F26 | Twelve schema-enum variants are unpinned by the v2 golden fixtures — including `ThemeId::Dark`, the default and therefore the value in most real settings files. A single payload holds one value per scalar field, so no fixture can cover them; add per-variant serialization assertions across the ten schema enums. Verified by mutation: renaming a covered variant fails a test, renaming `Dark` passes all 969 | review 036 | **before RFC-076 patch 4** |
