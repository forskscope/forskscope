...
| F61 | **REOPENED (review 065).** A tab opened via CLI startup args is still not persisted. The `ecc5a28` fix — `atomic_write_envelope` creating its target's parent directory — is a real fix for a real bug and is kept, but it is **not F61's cause**. Verified against the fixed build on a real desktop, isolated `XDG_CONFIG_HOME`: with the config directory pre-created, no `session.json` after 14s; with a genuinely fresh profile, no `session.json` **and the `forskscope/` directory was never created** — decisive, since `create_dir_all` is the first statement of `atomic_write_envelope`, so **that function is never reached**. Closing the tab in the same run writes the file immediately, proving the process, the flag and the repository layer all work. So the write is not failing — it is never attempted. The implementer's `VirtualDom` harness *did* run the effect (it produced a genuine `NotFound` toast), so **harness and product disagree** and the fix was made against the harness; the test now passes while the product fails. Leading hypotheses, none established: the effect fires once before the async startup tab arrives and is never re-triggered; `session_write_disabled` is true during the only firing window; or the subscription does not register. Un-waivable Gate D blocker per RFC-078 | M5-B, reopened by review 065 | critical path |
